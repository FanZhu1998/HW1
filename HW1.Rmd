...
```{r Exercise#2}
help(Auto)
#Quantitative: mpg, displacement, horsepower, weight, acceleration and 
...
```{r Exercise#3}
ranges_Auto <- t(rbind(range(Auto$mpg),
...
```{r Exercise#4}
centers_Auto <- rbind(c(mean(Auto$mpg),
                        mean(Auto$cylinders),
...
```{r Exercise#5}
#library(ggplot2)
#ggpairs(Auto[,1:3],title = "Scatterplot Matrices", progress = FALSE)
#ggpairs(Auto[,c(1,4,5)],title = "Scatterplot Matrices", progress = FALSE)
#ggpairs(Auto[,c(1,6,7)],title = "Scatterplot Matrices", progress = FALSE)
```
...
7(e) The predicted mpg associated with a horsepower of 98 is 24.46708. The lower bound of the 95% confidence interval is 23.97308 and the upper bound of the interval is 24.96108. We can tell the dealer that with a horsepower of 98, the car would run 24.46708 miles per gallon of gasoline. A more stats-version of the confidence interval is that: if we take the samples repeatedly and computed the 95% confidence interval for each sample, 95% of the intervals would contain the true population mean. 
